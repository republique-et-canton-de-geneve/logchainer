...
public class FolderService {
    ...
    private static final Logger LOG = LoggerFactory
	    .getLogger(FolderService.class.getName());
	...
	Files.move(Paths.get(pDir + "/" + pFile ), 
		Paths.get(getTmpProperty() + "/" + pFile), 
		new CopyOption[]{});

	LOG.debug("file successfully moved to directory : " + getTmpProperty() + "/" + pFile);
  
	return getTmpProperty() + "/" + pFile;
    }
     ...
     */
    private static String getTmpProperty() {
	String tmp;

	try {
	    tmp = AppConfiguration.load().getProperty("tmpDirectory");
